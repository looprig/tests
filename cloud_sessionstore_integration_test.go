...
func newCloudWorld(t *testing.T, ctx context.Context, cfg cloudConfig, options orchestrationtest.PooledWorldOptions) *cloudWorld {
	...
	// One DISTINCT, empty journal backend per tenant: harness files a journal
	// under a single-tenant layout, so two tenants on one backend would share
	// it. NewPooledWorld serves both tenants when none are named.
	tenants := options.Tenants
	if len(tenants) == 0 {
		tenants = []sessionwire.TenantID{orchestrationtest.PooledTenantA, orchestrationtest.PooledTenantB}
	}
	options.JournalBackends = map[sessionwire.TenantID]*storage.Composite{}
	for _, tenant := range tenants {
		backend := openCloudBackend(t, ctx, cfg, newCloudDeployment(t))
		world.journals[tenant] = backend
		options.JournalBackends[tenant] = backend.Composite
	}

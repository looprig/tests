...
// PooledHostConfig chooses one stress Host's identity and sizing. Every zero
// sizing field keeps the lanes' default.
type PooledHostConfig struct {
	ID         sessionwire.HostID
	Generation uint64

	// Capacity is how many sessions the Host may hold resident.
	Capacity uint64
	// WarmTTL is how long an idle pooled session stays resident before the
	// Host warm-releases it (host >= v0.7.0).
	WarmTTL time.Duration
	// MaxBindingsPerLink and MaxBindings bound the Host's HostLink routing
	// table. They must be sized with Capacity or a Factory's bind is refused.
	MaxBindingsPerLink int
	MaxBindings        int
	// CommandQueueSize is the Host's local command buffer; the stress asserts
	// the scraped depth never exceeds it.
	CommandQueueSize int
}

// StartSizedPooledHost composes, starts and serves one pooled Host with the
// supplied sizing, over the world's shared plane.
func StartSizedPooledHost(tb TB, ctx context.Context, world *PooledWorld, cfg PooledHostConfig) *PooledHost {
	tb.Helper()
	return startHostTuned(tb, ctx, world, cfg.ID, cfg.Generation, "", nil, pooledHostTuning{
		capacity:           cfg.Capacity,
		warmTTL:            cfg.WarmTTL,
		maxBindingsPerLink: cfg.MaxBindingsPerLink,
